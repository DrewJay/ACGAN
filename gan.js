 ...
 * 
 * @param latentSize - The size of "latent space" vectors - or the noise.
...
function buildGenerator(latentSize) {
    ...
    // would be dense layer that takes [1] input and converts it into [latentSize]
 ...
 *
 * @param latentSize - Latent vectors size.
...
async function trainDiscriminatorOneStep(
    ...
    const [x, y, auxY] = tf.tidy(() => {
        ...
        const y = tf.tidy(() =>
            tf.concat([
                    tf.ones([batchSize, 1]).mul(SOFT_ONE),
...
async function trainCombinedModelOneStep(batchSize, latentSize, combined) {
    const [noise, sampledLabels, fakeTrick] = tf.tidy(() => {
        ...
        const fakeTrick = tf.tidy(() => tf.ones([batchSize, 1]).mul(SOFT_ONE));
...
function parseArguments() {
	const parser = new argparse.ArgumentParser({
		description: 'Generative Adversarial Network generating numeric images.',
		addHelp: true
	});
