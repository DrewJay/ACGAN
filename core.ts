...
const constants = {
	...
	defaults: {
		...
		learningRate: 0.0002,
		adamBeta: 0.5,
		generatorSavePath: './dist/generator',
		logDir: './log',
		classes: 10,
		squareDim: 28,
	},
...
function buildDiscriminator() {
    ...
    const image = tf.input({ shape: [imageSize, imageSize, 1] });
...
function buildCombinedModel(
    ...
    const [fakeScore, auxScore] = discriminator.apply(fakeImage);
...
async function trainDiscriminatorOneStep(
    ...
    const [x, y, auxY] = tf.tidy(() => {
        ...
        // Generate numeric class for each latent vector created before. It is random number
        // between 0 and numberOfClasses. It is used as an access index in embedding layer.
        ...
        const y = tf.tidy(() =>
            tf.concat([
                    tf.ones([batchSize, 1]).mul(constants.softOne),
 ...
 * on losses generated by current state of discriminator. Primary purpose
 * of discriminator is to determine the "realness" score. Since this GANN
 * is auxiliary - it also predicts image class. Adding more parameters to
 * training images along the realness score helps to accelerate the process
 * by making the final loss converge faster to zero.
 *
 * @param batchSize - Batch size.
 * @param latentSpaceSize - Latent vectors size. 
...
async function run() {
	...
	numberOfClasses = args.classes;
	imageSize = args.squareDim;
	...
	if (numberOfClasses === args.classes && imageSize === args.squareDim) {
